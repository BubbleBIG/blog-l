...
class Verify {
    ...
    public function entry($id = '') {
        // 图片宽(px)
        $this->imageW || $this->imageW = $this->length*$this->fontSize*1.4 + $this->length*$this->fontSize/2;
        // 图片高(px)
        ...
        // 验证码字体随机颜色
        $this->_color = imagecolorallocate($this->_image, mt_rand(1,50), mt_rand(1,50), mt_rand(1,50));
        //曲线
        $this->_color1 = imagecolorallocate($this->_image, mt_rand(118,150), mt_rand(118,150), mt_rand(118,150));
        // 验证码使用随机字体
    ...
    private function _writeCurve() {
        ...
        for ($px=$px1; $px<=$px2; $px = $px + 1) {
            if ($w!=0) {
                ...
                while ($i > 0) {
                    imagesetpixel($this->_image, $px + $i , $py + $i, $this->_color1);  // 这里(while)循环画像素点比imagettftext和imagestring用字体大小一次画出（不用这while循环）性能要好很多
                    $i--;
                }
            }
        }
        ...
        for ($px=$px1; $px<=$px2; $px=$px+ 1) {
            if ($w!=0) {
                ...
                while ($i > 0) {
                    imagesetpixel($this->_image, $px + $i, $py + $i, $this->_color1);
                    $i--;
                }
            }
        }
